...
const ProductList = () => {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3000/api/products-and-categories") // เชื่อมต่อ API backend ที่ส่งข้อมูลทั้งสินค้าและหมวดหมู่
      .then((response) => response.json())
      .then((data) => {
        setProducts(data.products); // ตั้งค่าข้อมูลสินค้า
        setCategories(data.categories); // ตั้งค่าข้อมูลหมวดหมู่
      })
      ...
      <div className="product-list-container">
        <div className="filter-sidebar">
          ...
          <form>
            ...
            {/* ฟังก์ชันกรองราคาและสถานะยังคงเหมือนเดิม */}
            <div className="mb-4">
              <label className="block text-sm font-medium mb-2">Price Range</label>
              <div className="flex gap-2">
                <input
