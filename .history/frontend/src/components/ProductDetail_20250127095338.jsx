...
const ProductDetail = () => {
  const { id } = useParams(); // ดึงค่า id จาก URL
  const [product, setProduct] = useState(null);
  const [mainImage, setMainImage] = useState(""); // สำหรับจัดการภาพหลัก
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/api/products/${id}`) // API สำหรับดึงข้อมูลสินค้าตาม id
      ...
      .then((data) => {
        setProduct(data);
        setMainImage(`/products${data.image_path}`); // ตั้งค่าภาพหลักเริ่มต้น
      })
      .catch((error) => console.error("Error fetching product:", error));
  }, [id]);
  ...
  return (
    <div className="container mx-auto p-4 flex flex-col md:flex-row gap-6">
      {/* รูปภาพสินค้า */}
      <div className="md:w-8/12 flex flex-col justify-center items-center relative">
        {/* ภาพหลัก */}
        <div
          className="image-container"
          onMouseMove={(e) => handleZoom(e)} // เรียกฟังก์ชันเมื่อเมาส์เลื่อนไปบนภาพ
        ...
        >
          <img
            src={mainImage} // ใช้ภาพหลักที่เปลี่ยนได้
            alt={product.name}
            className="main-image rounded-lg shadow-lg object-contain w-full h-[60vh] max-w-full max-h-[70vh]"
          />
        </div>
        ...
        {/* รูปภาพเพิ่มเติม */}
        <div className="additional-images flex gap-4 mt-4">
          {product.additional_images &&
            product.additional_images.map((image, index) => (
              <img
                key={index}
                src={`/products${data.image_path}`}
                alt={`Additional ${index}`}
                className="additional-image cursor-pointer rounded-lg shadow-md w-[80px] h-[80px] object-cover"
                onClick={() => setMainImage(`/products${image}`)} // เปลี่ยนภาพหลักเมื่อคลิก
              />
            ))}
        </div>
      </div>
      ...
      <div className="md:w-6/12 space-y-6">
        ...
        <ul className="list-disc ml-6 space-y-2">
          <li>
            <strong>Installation Type:</strong> {product.installation_type || "N/A"}
          </li>
          <li>
            <strong>Screen Size:</strong> {product.screen_size || "N/A"}
          </li>
          <li>
            <strong>Resolution:</strong> {product.resolution || "N/A"}
          </li>
          <li>
            <strong>Brightness:</strong> {product.brightness || "N/A"}
          </li>
          <li>
            <strong>Connectivity:</strong> {product.connectivity || "N/A"}
          </li>
          <li>
            <strong>Operating System:</strong> {product.operating_system || "N/A"}
          </li>
        </ul>
      </div>
    </div>
